function [sector_endpoints, num_sectors] = generate_sector_points_3(sector_vector, num_sector_weights, min_sector_spacing, have_min_sector_spacing)
...
if num_endpoints == 0
    % Should this colony be cured
    cured_rand = rand;

    % If yes, then return a list with only one point
    if cured_rand >= 0.5
        sector_endpoints = 2*pi*rand;
    end

    % If not, then nothing else is needed
    return
end
